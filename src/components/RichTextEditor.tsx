...
export interface RichTextEditorRef {
  insertAtCursor: (content: string | Node) => void;
  insertVariableAtCursor: (varKey: string) => void;
  focus: () => void;
}
...
const RichTextEditor = forwardRef<RichTextEditorRef, RichTextEditorProps>(
  ({ value, onChange, variables = [], onAddVariable }, ref) => {
    ...
    const insertAtCursor = useCallback(
      (content: string | Node) => {
        ...
        if (!range || !editorRef.current.contains(range.commonAncestorContainer)) {
          ...
          range.collapse(false); // Move to end if no valid selection inside editor
        }
        ...
        const lastChild =
          nodeToInsert.nodeType === Node.DOCUMENT_FRAGMENT_NODE
            ? nodeToInsert.lastChild
            : nodeToInsert;
    ...
    useImperativeHandle(
      ...
      [insertAtCursor, insertVariableAtCursor],
    );
    ...
    const handlePaste = (e: React.ClipboardEvent<HTMLDivElement>) => {
      // Intercept paste for Shift + Paste (Ctrl/Cmd + Shift + V)
      const native = e.nativeEvent as unknown as { shiftKey?: boolean };
      const isShiftPaste = Boolean(native && native.shiftKey);
      if (isShiftPaste) {
        e.preventDefault();
    ...
    const getActiveTableContext = () => {
      ...
      while (node && node !== editorRef.current) {
        if (node.nodeName === 'TD' || node.nodeName === 'TH') {
          ...
          const table =
            (row.parentElement?.parentElement as HTMLTableElement) ||
            (row.parentElement as HTMLTableElement);
    ...
    const addTableCol = () => {
      ...
      rows.forEach((r) => {
        ...
        const newCell = document.createElement(
          r.parentElement?.nodeName === 'THEAD' ? 'th' : 'td',
        );
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            ...
            <Tooltip title="Paste Without Formatting (Ctrl+Shift+V)">
              <Button
                size="small"
